...
//
// inc 602 (#82, c3): the job lifecycle + done-state rendering now come from the shared useScopedAsk /
// ScopedAskResult shell (20d_scoped_ask.jsx), which axis-scoped Ask also rides — one Ask interaction, two scope
// selectors. This file keeps only the reader-specific scope (single paper) + its header/gate/evidence handoff.
...
function ReaderAsk({ paperId, title, onOpenCitation, onSaveHighlight, onOpenSettings }) {
  ...
  const { state, run, cancel } = useScopedAsk();  // shared lifecycle (inc 602)
  ...
  const ask = useCallback(() => {
    const q = query.trim();
    if (!q || !valid) return;
    run(readerAskRequest(paperId, q));
  }, [query, valid, paperId, run]);
          ...
          <div className="axis-modal reader-ask-modal" role="dialog" aria-label="Ask this paper"
            ...
            {state.status === "done" && result &&
              <ScopedAskResult result={result} onOpenCitation={openEvidence} onSaveHighlight={onSaveHighlight}
                coverageNoun="from this paper"
                emptyNote="The generator returned no sentences — your question may not be addressed in this paper." />}
          </div>
        </div>}
    </>
  );
