...
function TagsRow({ paperId, initialTags, onFilterToTag, onTagsChanged, readOnly }) {
  const immutable = !!readOnly || isDemoMode();
      ...
      <div className="detail-tags-chips">
        {tags.map(t => (
          <span key={t.id}
            ...
            {!immutable && !tagIsSystemFact(t.source) && <button className="tag-chip-dot" title="Set a color for this tag"
              onClick={() => setPicking(p => (p === t.id ? null : t.id))}>●</button>}
            {!immutable && !tagIsSystemFact(t.source) && <button className={"tag-chip-lock" + (t.locked ? " on" : "")}
