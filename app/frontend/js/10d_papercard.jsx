...
// #57: an explicit, always-visible "Delete" button per card. (inc 585 first hid it behind a ⋯ overflow menu;
// the reporter — Bella — pointed out that hiding the delete action recreates the exact discoverability problem
// it was meant to fix, so inc 587 promotes it to a plain labelled control.) Deleting is safe to surface plainly
// because it routes through the shared `onTrash` primitive, which ALWAYS confirms (window.confirm) and only
// soft-deletes to Trash (restorable) — so there is no accidental-one-click-delete risk. Neutral at rest (matches
// the foot-row controls), red on hover (`--danger`, DESIGN §4 destructive); labelled for screen readers; stops
// propagation so it never selects/opens the surrounding card.
function PaperTrashButton({ paperId, onTrash }) {
  return (
    <button type="button" className="paper-trash"
      aria-label="Delete this paper (moves it to Trash — restorable)"
      title="Delete — moves this paper to Trash (you confirm first; restore any time)"
      onClick={(e) => { e.stopPropagation(); onTrash(paperId); }}>Delete</button>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="paper-foot">
        ...
        {onTrash && <PaperTrashButton paperId={p.id} onTrash={onTrash} />}  {/* #57: explicit per-card delete */}
      </div>
    </div>
  );
}
