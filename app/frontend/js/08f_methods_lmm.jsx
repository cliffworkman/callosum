        ...
        <div key={c.key} className={"bayes-check-item" + (c.status === "not-applicable" ? " lmm-na" : "")}>
          ...
          {c.evidence &&
            <EvidenceQuote text={c.evidence} match={c.evidence} label="Evidence" className="bayes-check-ev"
              ...
              onOpen={c.page != null ? () => onOpen(c, `lmm-check:${c.key}`) : null}
          ...
          {c.evidence && <EvidenceTrail detector="Mixed-model reporting" matched={c.evidence}
            precision={c.coordinate_precision} hasSourcePage={c.page != null} page={c.page}
            caveat="Reporting checklist signals are text detections only; they do not judge model correctness." />}
        </div>
      ))}
