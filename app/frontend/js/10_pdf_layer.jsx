...
function PaperList({ state, query, onQuery, selected, onSelect, page, onPage, total, onOpenPdf,
  ...
  const showMetaChip = !trashView && metaFlagged > 0 && librarySignalFilter !== "meta-incomplete";
  const showBayesChip = !trashView && bayesFlagged > 0 && librarySignalFilter !== "bayes-flagged";
  return (
          ...
          {!readOnly && <span className="lib-head-actions">
            ...
            {(showStatcheckChip || showRetractionChip || showTransparencyChip || showLmmChip || showMetaChip || showBayesChip) &&
              <span className="lib-chip-group lib-chip-signals" title="Check signals — a check detected something concrete on these papers">
                ...
                {showMetaChip &&
                  <button className="trash-toggle meta-chip" onClick={onShowMetaFlagged}
                    title="Papers detectably a meta-analysis where the reporting checklist found ≥1 item not detected in the text — a completeness gap to inspect, never a verdict on the pooling">∑ Meta · {metaFlagged}</button>}
                {showBayesChip &&
                  <button className="trash-toggle bayes-chip" onClick={onShowBayesFlagged}
                    title="Papers detectably using Bayesian statistics where a reported Bayes factor didn't reproduce under the default prior, or the reporting checklist found a gap — a signal to inspect, never a verdict">B Bayes · {bayesFlagged}</button>}
              </span>}
            {showFindingsChip &&
