...
**Grounded citation gaps.** The **Citation gaps** panel looks beyond works your publications already cite. Click **Find citation gaps** to run an explicit, bounded OpenAlex scan: Callosum finds references shared by at least two of your confirmed publications, follows the public graph to other works citing those references, and excludes anything OpenAlex shows your scanned publications citing. Suggestions are ordered only by the visible number of shared references and source publications — never by a hidden importance or quality score. Expand **Why this surfaced** to see each shared reference and the exact publications of yours that cite it; click one of your publication titles to inspect its library record, or open the candidate/reference in OpenAlex. **Add** imports a DOI-backed candidate as a metadata record into the general library; **Dismiss** hides it from gap surfaces. A normal dashboard open reads the local snapshot and makes no network request; only **Find/Refresh gaps** queries OpenAlex. The coverage line states how many confirmed publications were scanned, whether the cap was reached, and why no-result output is not a certificate of completeness.
