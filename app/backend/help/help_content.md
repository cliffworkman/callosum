...
- **Insert current-section bibliography here** — places a local bibliography exactly at the caret. “Current section” means the nearest preceding Writer heading plus its nested lower-level headings, not a separately created Writer Section. The bounded live block contains only works cited there. Multiple section blocks can coexist with the full bibliography and update through the normal bibliography refresh. Use **Remove bibliography for current section** anywhere in that heading subtree to delete only its block. Remove section blocks before converting citation placement so conversion remains safely undoable.
