...
The three panes stay put; only the center changes as you switch workspaces. On a phone-width screen, the center pane uses a compact **Workspace** dropdown instead of the full desktop tab strip. **Status** remains beside it. Status collects running AI operations—both configured provider calls and installed local tools such as embeddings, semantic retrieval, NLI, OCR, and clustering—plus other lengthy operations that show progress. Routine **Library scans** and **WIP folder scans** are intentionally kept out because they run frequently; their progress remains visible where you started or configured them. Each Status row tells you whether the work is local or provider-backed, what stage it has reached, and how long it has been running. After enough comparable runs on this device, Status may add a cautious recent timing range or remaining-time estimate; sparse or provider-variable work stays elapsed-only, and an overrun is labeled **Taking longer than recent runs** instead of showing a false countdown. These local timing receipts contain duration and coarse configuration/workload shape only—never paper or prompt content—and old estimator versions are ignored. A Status row can be clicked to return directly to the relevant paper, workspace, panel, or dialog. The side panels resize with the vertical grips and collapse with the chevron next to each. Within a side pane the open section is remembered between sessions, and in-progress workspace tasks stay mounted while you switch away.
...
- For a whole reference list: in the Library, check the papers you want, then in the bulk bar open the **bibliography…** dropdown and pick a style — Callosum downloads a formatted `.html` bibliography (open it in your browser or word processor). This is the same local citation engine used by the word-processor adapters.
- In **Settings → Citation styles**, search the installed catalog by style, journal, discipline, acronym, or name; inspect a real fixed-example preview; choose a locale; and manage Favorites, Recent styles, and the application default. The **Repository** view searches the Citation Style Language project's public catalog only when you press **Search**; the catalog is downloaded from Zotero and your query is matched locally. Install a result directly, use **Install .csl** for a local file, or use **Import URL** for an explicit HTTPS download. Remote and local imports pass bounded checks, the official CSL 1.0.2 schema and macro rules, and the real citeproc engine; dependent repository styles bring in their required parent automatically. Each personal style shows whether it came from a local file, repository, URL, or another style, with source and check dates where available. **Check for updates** appears only for remote imports and makes a request only when pressed; an available update is installed from the exact bytes just validated. **Duplicate** creates a new independent personal style, including when the selected style depends on a parent, so later editing cannot change or rely on the original. Choose **Edit source** on an independent personal style to validate and preview unsaved CSL locally before saving. Bundled and dependent styles offer **Duplicate to edit** instead. A save retains the style's identity and refuses to overwrite a revision changed since the editor opened; documents using it receive the saved formatting on their next refresh. Re-importing the same canonical style detects an exact duplicate or asks before updating it; bundled styles cannot be replaced. A personal style can be downloaded as a portable `.csl` backup and reinstalled on another device with the same Callosum style id. To remove one, first choose another application default; export a backup before confirming because existing documents need the same style reinstalled to render again. New word-processor documents inherit the application default, while existing documents keep their embedded style and locale.
...
- To widen the pool, turn on **Also search beyond my library** before clicking **Suggest**. Callosum sends only the draft sentence/description you pasted to public metadata providers such as Crossref, PubMed, OpenAlex, and Semantic Scholar, then shows outside-library candidates in a separate section. It also uses the top local library matches as anchors for OpenAlex graph expansion and Semantic Scholar's recommendation engine, so an outside-library candidate may be labeled as **cited by**, **cites**, **related to**, or **recommended alongside** a locally relevant paper. These candidates are based on public metadata, abstracts, and graph/recommendation relationships — never a hidden relevance score — and if stance is shown, it is **abstract-level** and weaker than full-text library evidence.
...
- Not ready to decide? Click **Save for later** on an outside-library candidate to flag it for a second look — nothing is added or inserted, it's only queued. Review, add, or dismiss saved candidates anytime from **Discover → Search → Saved**. Saving is always an explicit click; nothing is queued automatically just because it was shown.
...
- **Add citation…** — the composer: search your library **live as you type**, add one or more results to the citation you're building, and watch a **real rendered preview** update as you go, before you click **Insert**. Building a grouped citation like `(Smith, 2020; Jones, 2021)` from scratch is one composer session, not insert-then-merge. Select an assembled source and click **Options…** to set a **locator** (page / chapter / section / … — a fixed vocabulary), a **prefix**/**suffix** (e.g. "see …"), or **suppress the author** / show **author only** — each applies to this citation only, your library record is never touched. **Move ↑ / ↓** reorders sources manually (a citation style with its own sort rule, like APA, may still reorder them at render time regardless — the preview always shows the real result, so you'll see this rather than be surprised by it). Check **Also search beyond my library** to widen the search the same way the in-app Cite panel does (see *Verified citation suggestions*) — off by default, opt-in each time; picking an outside-library result adds it to your library first, then cites it. With **Chicago notes and bibliography** selected, Insert creates a real Writer footnote/endnote when the caret is in the main document. Put the caret inside an existing configured note to add another independent live citation there; surrounding prose remains ordinary Writer text.
- **Suggest citation** — select the sentence you're writing; callosum suggests papers by relevance, with stance + a quote (and the same beyond-library option), the same engine as the in-app Cite panel. Select one or more in-library results (Ctrl/Shift-click) to insert them together as one grouped citation. **Details…** (available for a single selected in-library result) shows the full matched quote, the complete support/contrast/mention stance breakdown, a plain-language note on why it was retrieved, a weak-evidence warning when neither signal is strong, an editable page locator pre-filled from the match, and **Open in PDF** to jump straight to the source passage. Select one or more outside-library results and click **Save for later** to flag them for a second look without inserting or adding anything — review, add, or dismiss them anytime from the callosum app (Discover → Search → Saved).
...
- **Delete citation** — removes the citation, both the field and its rendered text. In a prose-bearing note, other live citations and prose remain; deleting the last live citation does not remove the note while prose remains.
...
- **Open cited work in callosum…** — opens the cited paper's page in the callosum app (a browser tab). For a grouped citation, choose the specific source first.
- **Go to bibliography entry…** — jumps to a cited source's stable entry in the full bibliography. For a grouped citation, choose among sources that currently have an entry. Excluded works and documents without a built full bibliography explain why no destination is available.
- **Refresh / renumber + bibliography** — re-render every citation and rebuild the bibliography (numeric styles renumber by position; run it after edits or after moving citations). For a large document, **Refresh citations only** leaves the bibliography untouched, while **Refresh bibliography only** leaves citation text untouched and works even when automatic bibliography rebuilding is paused. Place the caret inside a live citation and choose **Refresh citation at cursor** to update only that citation. Choose **Refresh current section** to update citations under the nearest preceding heading, including nested subsections, stopping at the next heading of the same or higher rank. Text before the first heading is a preamble section; a heading-free document is one section. Both targeted commands still render the full citation sequence for correct numbering, but do not alter citation fields outside their target or the bibliography. A document-wide pending-citations warning remains until you run a document-wide citation refresh. Large refreshes show progress in Writer's status bar; press **Esc** to cancel. Cancellation rolls every partial citation/bibliography write back, and a citation changed while rendering is underway is never overwritten by the stale result. Citeproc always receives the complete ordered document, but Writer updates only citation fields whose visible result changed and rebuilds the managed bibliography only when its exact text changed; an already-current refresh creates no undo entry.
- **Toggle automatic citation formatting** — switch to manual refresh mode for a large document. Citation inserts and edits remain structured live fields, but visible text waits for **Refresh / renumber + bibliography** or **Refresh citations only**; new pending inserts show `{citation}`. Turning automatic formatting back on affects later changes, so run one explicit refresh for existing pending changes. Bibliography auto-rebuilding is controlled separately. Whenever citation formatting or the bibliography is pending, Writer shows a non-dismissible **Callosum refresh pending** bar naming the stale surface(s); **Refresh pending** updates exactly those surfaces and removes the bar. The flags persist in the document, and the bar returns as soon as Writer reopens it. Moving, adding, deleting, or editing live citation fields through Writer's own commands is detected and marks the affected surfaces pending; unrelated prose edits do not. This observation is local and never renders or contacts Callosum until you choose a refresh command.
- **Citation style…** — search the same bundled and personal-style catalog used by Settings, choose a descriptive result and locale, and inspect a real fixed-example citation + bibliography preview before applying. The whole document then re-renders and the style appears in Recent. Install personal `.csl` files from the full manager in Settings; they appear in Writer automatically. A blank Writer document inherits the application default on its first citation; an existing document keeps its embedded style and locale. Note styles render new citations in native Writer notes with style-defined first, ibid, locator-aware ibid, near-note, and subsequent-note context. Ordinary Writer notes count in that native note sequence without being changed.
- **Note placement…** — choose **Footnotes** (the default) or **Endnotes** for new note-style citations. The choice is saved in the document. Callosum does not silently convert existing inline citations, footnotes, or endnotes; an incompatible style or placement switch is refused before the document or saved setting changes.
- **Convert citation placement…** — deliberately convert an eligible document between inline citations, footnotes, and endnotes while choosing the target style. Callosum previews the source, target, and citation count, then lets you apply one Writer Undo step or save a separate converted `.odt` copy. Unrelated Writer tracked changes are preserved exactly and Track Changes recording returns to its original state. Mixed placement, malformed fields, multiple live clusters in one note, notes containing user prose, and tracked changes that overlap a live citation, source citation note/anchor, Callosum conversion state, or the managed bibliography are refused without mutation. A tracked-change range Writer cannot expose safely is also refused.
- **Insert bibliography here** — moves the bibliography to the cursor (run it again elsewhere to move it there instead).
- **Bibliography heading…** — set a heading for this document, such as **Works Cited** or **References and further reading**. The heading is saved in the Writer file and applied immediately even when automatic bibliography rebuilding is paused. Submit a blank heading to restore **References**.
- **Toggle citation-to-bibliography links** — add or remove document-local navigation from each unambiguous single-work citation to its managed bibliography entry; use Writer's standard follow-link action to navigate. The setting is saved in the Writer file and applied immediately, including when automatic formatting/rebuilding is paused. Grouped citations and works excluded from the bibliography remain plain because they do not have one valid destination; use **Go to bibliography entry…** to choose a specific available source in a group whether this setting is on or off. Turning links off removes only Callosum's internal links; unrelated external links are preserved.
- **Toggle bibliography title/DOI links** — make DOI or URL text that the selected citation style already includes clickable. When the style omits that identifier, Callosum instead links a uniquely matched rendered title to the source DOI (preferred) or URL. This opt-in setting is saved in the Writer file and applies immediately, including when automatic rebuilding is paused. It never adds text or guesses through ambiguous or transformed titles; unsafe or malformed destinations remain plain. Turning it off removes only Callosum-managed links inside the bibliography, not your other hyperlinks.
...
- **Citation integrity preflight…** — everything **Document diagnostics…** reports, plus a fresh, on-demand retraction/correction re-check for every paper actually cited in this document right now — a real-time check, not the cached status shown elsewhere in the app. Handy right before submission. Never changes your document.
- **Citation coverage audit…** — two read-only checks over the papers actually cited in this document: **citation concentration** (how much your reference list leans on already-famous work, or on a few journals/institutions — the same descriptive signal as the in-app Citation Concentration audit; self-citation reads "not computed," since a live document has no stored author identity to check against) and a **structural scan for long uncited stretches** (3+ consecutive substantive paragraphs with no citation, shown with a preview — a neutral note about *where citations are absent*, never a claim that one is actually needed there). Recognizes both inline and footnote/endnote-style citations. Never changes your document.
- **Convert Zotero citations…** — detects Zotero-authored inline citations (the default LibreOffice storage Zotero uses) and converts them to live Callosum citations: matching an existing library paper by DOI/title/author where possible, or adding a new paper straight from the citation's own embedded reference data when it isn't in your library yet. A confirm dialog shows exactly what will happen before anything changes, including how many citations will convert and what's left untouched. Zotero's own generated bibliography is replaced with a Callosum-managed one. Two things Zotero can produce aren't supported yet and are reported, not guessed at or silently skipped: citations stored in "Bookmarks" mode (an uncommon Zotero setting) and note-style (footnote/endnote) Zotero citations.
- **Citations in this document…** — shows each unique cited work, occurrence count, missing/retraction status, and a jump-to-first-occurrence action. From the same snapshot you can exclude one cited work from the bibliography, add an uncited work as further reading, or choose **Set category…**. Ctrl/Shift-select several works to categorize them together. The picker reuses category names already in the Writer file and has explicit **Create new category…** and **Remove category** choices; a mixed-category selection starts on a safe no-op placeholder. Use **Category order…** to move named groups up/down or reset to alphabetical. Newly created groups follow the saved groups alphabetically until positioned, the active citation style's order is preserved inside each group, and unassigned entries stay last under **Other references**. Reopen the panel after document edits to refresh its list. A citation inserted via **Suggest citation** or **Insert evidence…** also carries a compact evidence record — click **View evidence…** on it to see the page and quoted passage that justified it.
- **Insert current-section bibliography here** — places a local bibliography exactly at the caret. “Current section” means the nearest preceding Writer heading plus its nested lower-level headings, not a separately created Writer Section. The bounded live block contains only works cited there. Multiple section blocks can coexist with the full bibliography and update through the normal bibliography refresh. Citation-placement conversion updates the full bibliography and every non-empty section block in one verified Writer Undo step; damaged or empty section blocks are refused before mutation. Use **Remove bibliography for current section** anywhere in that heading subtree to delete only its block.
- **Section bibliographies…** — lists complete section blocks in document order by owning heading and unique cited-work count. Jump to one, remove the selected block, or confirm **Remove all** as one Writer Undo step. Citations and the full bibliography do not change. Undo/Redo restores managed text and links; damaged blocks are reported and make bulk removal fail closed until **Document diagnostics…** is resolved.
- **Journal abbreviations…** — chooses how Writer supplies short journal titles when the active CSL style asks for them: use the abbreviation embedded in each library/citation record (the default), prefer Callosum's bundled MEDLINE/NLM match with library fallback, or force full journal titles. MEDLINE matching uses ISSN first and then the exact normalized journal title; unmatched titles remain full and are reported rather than guessed. The saved document setting immediately refreshes citations plus full and section bibliographies without rewriting embedded or library metadata. The NLM snapshot is bundled locally, so this makes no runtime network request.
...
- **Insert evidence…** — search any paper in your library (not just ones you've already cited), pick one of its saved PDF highlights, optionally type a claim and click **Check stance** to see how the highlighted passage relates to it (support/contrast/mention — the same local signal Suggest citation uses), then insert it as **quote only** (plain text, no citation), **quote + citation**, **paraphrase** (your saved note) **+ citation**, or a **structured card** (quote + note + citation together). An editable page locator pre-fills from the highlight's own page.
- **Insert CRediT statement** — inserts the contribution statement you built in **Work → CRediT → Send to LibreOffice**, as plain text at the cursor.
- **Insert statement…** — inserts one of the open-science disclosure statements you built in **Work → Statements → Send to LibreOffice** (data availability, code availability, preregistration, funding, conflict of interest, ethics, or AI use). If you've staged more than one, pick which to insert from a short list showing each one's text.
...
- **Build and insert a citation** — search repeatedly to assemble one or several works, reorder them, and set each work's locator, prefix, suffix, suppress-author, or author-only option before inserting one live citation. **Suggest from the sentence** remains a separate, fully local relevance/stance/quote path into the same assembly. Open **Details…** for the full matched passage, support/mention/contrast breakdown, plain-language retrieval reason, weak-evidence warning, editable auto page locator, and **Open in PDF**. Inserting a suggested work keeps only a 150-character matched-passage snippet plus page/chunk locator inside the Word document; inspect it later with **Citations in this document… → View evidence…**.
- **Insert saved evidence…** — search for any library paper, choose one of its saved highlights, and review the exact quote and your own saved note. You explicitly choose quote-only (clearly labelled **no citation**), quote + live citation, saved note + citation, or a quote/note card + citation. **Check stance** is optional and runs only when clicked through Callosum's local model; its support/mention/contrast result is a signal, not a verdict. Cited formats reuse the normal live citation and native-note behavior and retain only annotation/page identity plus a bounded audit snippet.
...
- **Convert Zotero citations…** — scan Zotero's current inline Word fields, inspect exact counts, and explicitly convert verified fields through the existing local citation resolver and Refresh path. Grouped citations and per-work locators/overrides survive; known existing papers are reused and unmatched embedded records become metadata-only local papers. Note-style, Bookmark-mode, malformed, oversized, or ambiguous Zotero material is reported and left under Zotero's control. A Zotero bibliography is replaced only when no unsupported Zotero material remains. Use **File → Save As** first: Word add-ins cannot make a backup or guarantee one-step rollback, and Word Undo does not remove local library records created before document mutation.
...
- **Citations in this document…** — list each unique cited work, occurrence count, missing/retraction status, and jump to its first occurrence. **Set category…** assigns one document-local bibliography label; check several works (or filter and **Select visible**) to set or remove one category across the batch with a single refresh. With at least two groups, **Category order…** moves groups up/down or resets them alphabetically. Citeproc order stays intact within each group, new/unranked groups fall back alphabetically, and unassigned entries remain last under **Other references**.
- **Insert current-section bibliography here** — place the cursor under a Word heading to create a bounded live block for that heading plus nested lower-level headings, stopping at the next peer/ancestor. Multiple blocks can coexist with the full bibliography and share the same categories/order; Refresh updates all of them. **Remove bibliography for current section** removes only the current heading's block. This first slice requires WordApi 1.6 and an in-text citation style; note-style membership is refused rather than guessed.
- **Link bibliography titles/DOIs to the web** — opt in per document to make DOI/URL text already rendered by the active style clickable. When the style omits that identifier, Callosum may instead link one uniquely matched single-source title to its DOI or URL. The full and current-section bibliographies share the setting; no text is added or changed, and unsafe, malformed, shifted, or ambiguous metadata remains plain. Turning it off removes only links inside Callosum-managed bibliography blocks, not hyperlinks in your prose.
- **Insert open-science statement…** — choose data/code availability, preregistration, funding, conflict of interest, ethics, or AI use; pick a canned starting phrase or write your own, and review every word. **Insert at cursor** places the exact draft as ordinary editable prose at the end of the selection. Callosum does not infer or verify these author assertions, and creates no citation field. The optional **Stage for other editors** action shares the draft through the same transient local handoff used by Work → Statements and LibreOffice; clearing staged text does not touch the document.
...
- **Citation coverage audit…** — run a local, read-only structural scan for stretches of three or more consecutive substantive paragraphs without a Callosum citation anchor. Inline and native-note citations count at their main-text paragraph; headings, short transitions, tables, and managed bibliographies break a stretch. Treat every result as a prompt to review, not a finding that a citation is required or that prose is unsupported. No model or backend request runs; WordApi 1.6 is required.
...
The card shows a publication count and doubles as a "show only my papers" filter (click the count badge). Newly imported papers that match are added automatically. The 🗑 button dismisses the card (your profile and confirm/reject choices are kept — Refresh rebuilds it). A no-match shows an honest "No OpenAlex author found for [name] — check the name/ORCID"; see below if this happens despite having a correct ORCID.

**A correct ORCID that still won't resolve?** OpenAlex keeps its own author records, separate from ORCID's — an ORCID iD only produces an exact match once *OpenAlex's own author profile* actually lists it, and that link can be missing even when your ORCID page itself is complete and correct (OpenAlex's index may simply predate the connection). Callosum handles this case automatically: if the ORCID lookup alone comes up empty, it falls back to a name search rather than reporting no match — but it labels that result as matched **by name, not ORCID**, so you can see at a glance that it's a lower-confidence match and worth double-checking it's really you. To fix the underlying gap so future refreshes match you by ORCID directly, check your author profile at [openalex.org](https://openalex.org) and connect your ORCID iD there if it isn't already (OpenAlex documents how on their site) — that's a one-time step on OpenAlex's side, not something Callosum can do for you.
...
**Grounded citation gaps.** The **Citation gaps** panel looks beyond works your publications already cite. Leave **All publications** selected, or select one or more research-domain chips to scan only the union of those domains. Each all/domain combination keeps its own bounded local snapshot, so switching back to a previously scanned scope does not query OpenAlex again. Click **Find citation gaps** or **Find scoped gaps** to run an explicit, bounded OpenAlex scan: Callosum finds references shared by at least two confirmed publications in that scope, follows the public graph to other works citing those references, and excludes anything OpenAlex shows the scoped publications citing. Suggestions are ordered only by the visible number of shared references and source publications — never by a hidden importance or quality score. Expand **Why this surfaced** to see each shared reference and the exact publications of yours that cite it; click one of your publication titles to inspect its library record, or open the candidate/reference in OpenAlex. **Add** imports a DOI-backed candidate as a metadata record into the general library; **Dismiss** hides it from gap surfaces. A normal dashboard open or scope switch reads a local snapshot and makes no network request; only **Find/Refresh gaps** queries OpenAlex. The coverage line names the active scope, states how many confirmed publications were scanned, whether the cap was reached, and why no-result output is not a certificate of completeness.

**Emerging citing topics.** The neighboring **Emerging citing topics** panel compares OpenAlex works that cite your confirmed publications in the last **three complete calendar years** with the preceding three years. Leave **All publications** selected or use the same multi-domain union controls, then click **Find emerging topics** / **Find scoped topics**. A topic appears only when its OpenAlex **primary topic** occurs in at least two recent citing works and its recent count exceeds the earlier count. Cards show the two counts and their plain difference; there is no hidden trend, importance, or quality score. Expand **Inspect the citing works behind both counts** to open every retrieved citing work used for that surfaced topic and see the exact publications of yours it cites. The result is a bounded description of retrieved OpenAlex records — not a forecast that the field itself is growing. Ordinary reads and scope switches use independent local snapshots; only explicit Find/Refresh queries OpenAlex. Coverage names both year windows, the active scope, DOI/topic omissions, and any publication or citing-work cap, so an empty panel never certifies that the citing landscape is static or complete.

**Authors citing your work.** This final grounded-prospection panel reuses the same bounded six complete years of citing-work records. An author appears only when their stable OpenAlex identity is present on at least **two retrieved citing works** that together cite at least **two of your confirmed publications**. Callosum excludes your own author identity and any author identity found as a coauthor on the checked OpenAlex records for your publications. Leave **All publications** selected or use the same multi-domain union controls, then click **Find citing authors** / **Find scoped authors**. The two visible counts are the entire ordering basis — there is no hidden “collaboration potential,” compatibility, availability, importance, or quality score. Open the author in OpenAlex, or expand **Inspect every citing work and publication connection** to follow every citing work and exact local publication link behind the counts. Each card also has a **Follow** quick-action — one click to follow that author (see *Following authors*) without a separate name/ORCID lookup. This is a private reading index, not a recommendation of a person. “No coauthorship found” is always bounded to the own-work authorships returned and checked by OpenAlex; coverage reports unresolved records, missing author ids, and publication/window/authorship caps, so it never proves that two people have never collaborated. Ordinary reads and scope switches are local-only; only explicit Find/Refresh queries OpenAlex.
...
There are several ways to run a synthesis:
...
- **Ask one paper.** In the PDF reader, the **✦ Ask** button asks a question grounded only in the paper you're reading, without leaving the reader.
- **Ask one axis.** On an axis card (in **Axes**), the **✦** button asks a question across just that axis's papers. It's the same ordinary Ask — the axis only sets *which* papers are eligible, it doesn't change how Ask reasons or verifies. Before running, a short panel shows the corpus so there are no surprises: **N papers · M with usable full text** for each of two tiers. **Assigned only** (the default) uses the axis's confident members (its manually added papers plus those the scorer placed at or above the cutoff); **Include uncertain** additionally pulls in members the scorer placed *below* the cutoff — candidates you might still confirm. ("Assigned" is the axis's similarity tier, not a statement that you've endorsed each paper.) Only papers with usable full text can be retrieved, and an axis Ask **never** falls back to the whole library — a bounded, sparser answer is the honest result when an axis is small.
...
**The Overview.** Above the verified claims, a synthesis shows a short **Overview** — a plain-language narration *of the verified claims*, labeled "Overview — synthesized from the verified claims below". It is **not** an independent authoritative summary: it restates only what was verified and adds no new facts, and **each Overview line links back to the verified claim(s) it restates** — click a line and the claim(s) it traces to flash below, each carrying its own quote, page, and confidence. Verified claims appear as soon as their complete evidence graph is saved; the supplementary Overview may briefly say **Generating overview…** and never blocks those claims. If Overview generation fails, the verified synthesis remains available and you can retry only the Overview. The Overview is requested only when AI generation is enabled (it requires data-egress consent, like the synthesis itself) and there is at least one verified claim to narrate; with egress off, the verified claims stand on their own.
...
In the **METHODS** pane (the right-hand panel), open the **Statistics** section; under **This paper** — with a paper selected — there's a **Check statistics** button. It scans the paper for inline APA-style statistical tests — `t(28) = 2.10, p = .04`, `F(2, 45) = 3.1, p < .05`, `r(30) = .42, p = .01`, `χ²(1) = 5.2, p = .02`, `z = 2.1, p = .03` — and now also reads eligible rows in detected PDF, JATS/XML, HTML, DOCX, and ODT tables. It **recomputes** the p-value from the reported test statistic and degrees of freedom, then shows where the reported and recomputed values disagree. It's the statcheck method: a "spellchecker for statistics."

This runs **entirely on your machine** — pure computation over local extracted text and attachments, no AI and no network. Each prose result shows the **verbatim matched text** and nearby extracted-text context. Each table result is visibly labeled **TABLE**, shows the complete row plus its headers, and identifies the table/row number. Both show the **recomputed p**, with a status pill: **consistent** (green), **inconsistent** (amber — the values disagree), or **decision error** (amber — the disagreement flips significance at p = .05). A count summary reads "N checked · M inconsistent · K decision errors" and names how many came from tables — these are plain counts, never a hidden "reproducibility score." The coverage line reports attachments, detected tables, and rows scanned, plus whether a safety cap made coverage partial. Click a located result to open the PDF source. Prose may receive an exact passage highlight when Callosum can locally locate it; a PDF table row opens at the retained row region instead of pretending the reconstructed text is an exact quote.
...
- It reconstructs a table result only when the headers unambiguously identify the **test/type, degrees of freedom, statistic, and p-value** (or one cell already contains a complete APA result). Ambiguous, unlabeled, multi-p-value, or incomplete rows are skipped rather than guessed. Bayesian reporting and confidence-interval-only results remain invisible. **A clean result is not a clean bill** — it means nothing was surfaced by this specific check.
- It needs a local PDF or supported full-text attachment. PDF extraction and table detection can garble or miss symbols and layouts, which is why the exact row/headers or matched prose are always shown — so you can inspect the evidence yourself.

Evidence snippets show their source precision before you click: **exact highlight**, **region**, **page only**, or **no source page**. Open **Evidence trail** under a Methods evidence item to see the detector name, matched text, page, anchor note, and the boundary/caveat for that detector. Statcheck and the Bayesian, mixed-model, meta-analysis, and transparency auditors use the same source-jump contract for their evidence snippets: if the matched text can be locally located in the PDF on the expected page, Callosum opens with an exact highlight; otherwise it opens the source page at region precision. When the evidence came from a secondary PDF attached to the paper, the jump opens that exact PDF rather than the primary one. Evidence from a non-PDF attachment never sends that attachment to the PDF route. A region jump is navigation help, not an exact passage claim.
...
- A **fact** is structured metadata established about the paper (for example, that it has been retracted or carries an explicit correction record). It's one row in Critique's **"What the checks surfaced"** list — a label + a status, not something for you to act on.
...
## Checking retractions and correction records
Citing a retracted paper is a real hazard, while an explicit correction is useful positive evidence that a work participates in scientific self-correction. In the **Library** header, click **Integrity ↻**: Callosum refreshes the local **Retraction Watch** mirror when available, then checks each paper's DOI against Retraction Watch, **Crossref**, and **OpenAlex** (public metadata, no AI). If the Retraction Watch refresh is unavailable, the check still runs against the existing local mirror and the other registries, and the result says so.
...
- A paper with an explicit correction record gets a green **CORRECTION** badge on its library card. **Details → Positive integrity** names the registry source and date when available and links directly to the correction record. The same fact appears under **Tags → System facts** as **Correction**.
...
- A paper that was checked and **nothing** was found still gets its own "Retraction status" row. The absence of a green correction badge means only that these registries did not surface an explicit correction relation—not that the work has never been corrected. A paper with **no DOI** has no such row, since it was never checked.

New papers are also checked **automatically as they're imported** (a scan or a citation-file import), so a freshly added retraction or correction record appears without waiting for a batch run.
...
The **Gaps & overlooked** button in **Discover → Search** opens one window covering the two ways to find relevant work you don't have yet — a **Gaps** facet and an **Overlooked** facet, switched with the toggle at the top (see *Finding work the field overlooked* below for the Overlooked facet). The **Gaps** facet finds works **related to several of your papers** that aren't in your library yet — in two directions you can toggle between:
...
## Saved
The **Saved** button in **Discover → Search** opens the queue of outside-library citation suggestions you've explicitly flagged with **Save for later** while writing — from either the in-app Cite panel or LibreOffice's Suggest citation dialog (see *Verified citation suggestions* / *Citing in LibreOffice Writer*). Nothing lands here automatically; only a deliberate click queues a suggestion.

Each saved row shows the exact same title, evidence, and reason you saw when you flagged it, plus the draft sentence it came from. **Add** imports it into your library as a metadata-only record (the PDF stays the separate "Acquire OA copy" step); **Dismiss** removes it from the queue without touching your library. A suggestion that's already in your library (added any other way) drops out of the queue on its own.

<!-- section: overlooked-lens -->
## Finding work the field overlooked
The **Overlooked** facet of **Discover → Search → Gaps & overlooked** (toggle to it at the top of the window) looks, **per axis**, for external works that are **relevant to that axis but under-cited for their year** — work you may be missing because the field overlooked it, not because it's weak. (This is different from the **Find overlooked work** action on a single paper's citation-concentration audit, which is about that one reference list's omissions; this one is a library-wide discovery lens tied to an axis.)
...
## Building open-science disclosure statements
Most journals now also ask for a handful of standard disclosures alongside your manuscript: **data availability**, **code availability**, **preregistration**, **funding**, **conflict of interest**, **ethics**, and **AI use**. The **Work → Statements** tab helps you build each one, right next to CRediT.

Like CRediT, it is an **authoring aid, not a verifier.** Callosum offers common starting phrasing for each statement — it never asserts a fact about your study (that you received particular funding, had ethics approval, used AI, etc.) on your behalf. You are the source of truth for every one of these.

How to use it:

- Each of the 7 sections has a row of short phrase buttons (e.g. **"Openly available"**, **"No specific funding"**, **"None declared"**) — click one to drop a full starting sentence into that section's box, then edit the bracketed placeholders (like `[repository name]` or `[Funder name]`) to fit your study. If the box already has text you've typed or changed, callosum asks before replacing it, so a stray click never discards your own wording.
- Every box is otherwise a plain text field — type your own statement from scratch if none of the phrases fit.

Getting it into your manuscript:

- **Copy** puts that section's text on the clipboard to paste anywhere.
- **Send to LibreOffice** stages it for the **LibreOffice** Callosum add-on. You can stage more than one statement at once (e.g. Funding and Ethics both), since each is tracked separately. Then in Writer run **Callosum → Insert statement…** — if only one statement is staged it's ready to insert, and if several are staged you pick which one from a short list showing each one's text. (If you edit a box afterward, re-send it — the staged copy is cleared so you never insert a stale statement.)

<!-- section: discover-search -->
## Finding new papers (Discover)
The **Discover → Search** tab searches the wider literature by keyword, title, or author — so you can find papers *outside* your library and pull them in. It searches public bibliographic metadata across **Crossref** (which also covers bioRxiv/medRxiv preprints), **PubMed** (biomedical), **arXiv**, **Europe PMC**, and **PsyArXiv** — every source that offers a real keyword search. Leave the source dropdown on **All sources** to merge results from every provider (this queries all of them, so it's a little slower — pick a single provider when you want a fast, focused search), or choose one such as **arXiv** or **Europe PMC** when you deliberately want that source only. A paper indexed in more than one searched source shows multiple source pills. The only thing that leaves the machine is your search terms — this is **not** the AI/Gemini egress. (bioRxiv/medRxiv aren't separate Search options — they have no keyword-search API — but their preprints are covered through Crossref.)
...
Recent Search queries are stored in this browser only. Use **Recent searches** to re-run one with its saved source setting, **Clear history** to forget that list, or the **×** inside the search box (it appears once you've typed) to empty the active query and results without touching saved papers.
...
- **Follow a source** — the default source type is **Journal**: type a **journal title** like `Nature Neuroscience` and, as you type, the box suggests **journals already in your library**. Click **Follow** to add it. You can also switch the source type to a **preprint archive category** — **bioRxiv** / **medRxiv** (a subject like `neuroscience`), **arXiv** (a category like `cs.AI`), or **PsyArXiv** (a keyword) — a **keyword literature search** on **PubMed** (a query like `CRISPR off-target`) or **Europe PMC**, or **Author** — type a name or an ORCID iD (Callosum detects which one you typed) to follow a specific person and see their new work as it's published. You can also follow an author directly from **My Publications**, using the one-click quick-action on an already-identified citing-author's card. Each followed source shows as a chip (with its source tag) you can remove with its **×**. Once you have enough followed sources to fill more than one row, they're capped to a single line with a **…** button that opens the full list.
- **Suggest** opens a window whose top-level tabs name the *kind of source* — **Journal**, **Rxiv Categories**, **Keyword Search**, and **Author** — with a subtab per provider underneath where there's more than one. **Journal** lists every journal you already have papers from, ranked by how many papers you have from each. **Rxiv Categories** has subtabs for **bioRxiv**, **medRxiv**, **arXiv**, and **PsyArXiv**: the category archives list every category with any that match one of your axes or tags listed first (the match is named, never a hidden score), and PsyArXiv (which has no fixed categories) suggests keywords drawn from your Search history, axes, and tags. **Keyword Search** has subtabs for **PubMed** and **Europe PMC**, each suggesting queries from your recent Search history, your axes, and your tags. **Author** lists authors who recur across your library, ranked by how many papers you have from each — excluding you and anyone you already follow. Every suggestion is a plain, inspectable tally of your own library, never a quality ranking or a recommendation; if a provider has nothing to suggest yet, it says so rather than showing a blank. Click **Follow** on any suggestion to add it in one step — the same source you'd get by adding it manually.
...
- **AI features:** the AI section is **one provider list**. Choose **Local AI → Set up Local AI** to download and verify Callosum's supported model, prepare it on this device, and make it the active provider. No API key, provider account, endpoint, Ollama, terminal, or manual model download is required. Setup names each phase and shows real download progress plus an approximate ETA when measurable. Keep Callosum open, but you may choose **Continue in background**: setup keeps running and **Status → Setting up Local AI** returns to the live setup screen. Once it says **Local AI: Ready**, compatible generative features use that local provider throughout the app. A local failure is reported locally; Callosum never silently sends the request to Gemini, OpenAI, Anthropic, or another cloud provider.
...
**Off by default.** Citing from Google Docs or Word on the web requires your library to be reachable from the internet (their cloud-hosted add-ons cannot reach your computer directly). **Settings → Remote access** is the opt-in: turning it on generates an **access token** (shown once — copy it into the add-on) and requires that token on every request. In the packaged app, **Start Quick Tunnel** launches an app-owned `cloudflared` connector and shows its temporary URL; Stop or app exit removes it. A Quick Tunnel exposes every route, so that access token is its sole boundary. Callosum targets a separate fail-closed backend: turning Remote access off closes it even if the connector has not stopped yet. Source checkouts retain `python tools/run_tunnel.py --quick`; advanced users can configure the stable named tunnel with a cite-only ingress. See `adapters/googledocs/README.md` for full setup. While Remote access is off, nothing is exposed and local use is unchanged. **If you lose the token and get locked out**, callosum shows a recovery panel: paste the token, or use the one-time local-file code to turn Remote access off. As a last resort you can restart with `CALLOSUM_DISABLE_REMOTE_ACCESS=1` set.
