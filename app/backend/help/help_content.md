...
- **Citation style…** — pick a CSL style (apa, ieee, nature, …) and locale; the whole document re-renders. Chicago notes and bibliography renders new citations in real footnotes, including first/subsequent-note context. Callosum does not silently convert existing inline citations to notes (or notes back to inline), and endnotes are not supported yet; an incompatible switch is refused before the document or saved style changes.
