...
- In **Settings → Citation styles**, search the installed catalog by style, journal, discipline, acronym, or name; inspect a real fixed-example preview; choose a locale; and manage Favorites, Recent styles, and the application default. **Install .csl** validates a local CSL file before storing it as a personal style. Re-importing the same canonical style detects an exact duplicate or asks before updating it; bundled styles cannot be replaced. New word-processor documents inherit the application default, while existing documents keep their embedded style and locale.
