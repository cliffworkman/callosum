...
- **Citations in this document…** — shows each unique cited work, occurrence count, missing/retraction status, and a jump-to-first-occurrence action. From the same snapshot you can exclude a cited work from the bibliography, add an uncited work as further reading, or choose **Set category…** for the selected work. Categories are saved in the Writer file and shown alphabetically while the active citation style's order is preserved inside each category. When at least one visible work is categorized, unassigned entries stay visible under **Other references**; submit a blank category to remove an assignment. Reopen the panel after document edits to refresh its list.
