...
- **Toggle bibliography DOI/URL links** — make DOI and URL text that the selected citation style already includes in bibliography entries clickable. This opt-in setting is saved in the Writer file and applies immediately, including when automatic rebuilding is paused. It does not add missing identifiers or change the rendered bibliography; unsafe or malformed destinations remain plain. Turning it off removes only Callosum-managed links inside the bibliography, not your other hyperlinks.
