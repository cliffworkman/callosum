...
- **Citation style…** — search the same catalog used by Settings, choose a descriptive result and locale, and inspect a real fixed-example citation + bibliography preview before applying. The whole document then re-renders and the style appears in Recent. A blank Writer document inherits the application default on its first citation; an existing document keeps its embedded style and locale. Chicago notes and bibliography renders new citations in native Writer notes, including first/subsequent-note context.
