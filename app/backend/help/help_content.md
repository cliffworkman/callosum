...
## Seeing how a paper is cited
The **How this paper is cited** section (METHODS, with a DOI'd paper selected) shows how the *later literature* has responded to a paper — do subsequent papers **support** it, **contrast** it, or just **mention** it? Select a paper, open the section, and click **Fetch citations**: Callosum sends only the paper's DOI to **Semantic Scholar** (public metadata) to get the actual **citing sentences**, then classifies each one's stance **on your machine** with its own NLI model — your library text never leaves.
