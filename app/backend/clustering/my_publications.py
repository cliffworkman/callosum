...
def resolve_my_publications(conn: Connection, *, author_client, force: bool = False) -> dict[str, Any]:
    """Resolve + (re)write the My Publications axis memberships. Returns a status summary. All-in-one on one
    connection (unchanged for direct callers); the async job runs ``_resolve_fetch`` (lock-free) then
    ``_resolve_persist`` (a short write) separately so its OpenAlex fetches don't hold the write lock (inc D)."""
    status, author, works = _resolve_fetch(conn, author_client=author_client, force=force)
...
def _resolve_persist(conn: Connection, author, works) -> dict[str, Any]:
    """Persist phase (inc D): set the author id + rewrite the AUTO memberships (preserve manual/confirmed). Runs in
    its OWN write transaction (a fresh snapshot after the lock-free fetch phase)."""
