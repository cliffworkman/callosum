...
class OpenAlexSourcesClient:
    ...
    def fetch_topic_for_subject(self, conn: Connection, subject: str) -> str | None:
        ...
        try:
            return self.fetch_topic_for_subject_strict(conn, subject)
        except OpenAlexResponseUnavailable:
            return None

    def fetch_topic_for_subject_strict(self, conn: Connection, subject: str) -> str | None:
        """Resolve a subject or a proven absence, raising when OpenAlex is unavailable."""
        subject = (subject or "").strip()
        if not subject:
            return None
        cache_key = "topicsearch:" + subject.lower()
        body = self._get(
            conn, "/topics", {"search": subject, "per-page": "1"}, cache_key, {"subject": subject}, strict=True
        )
        results = (body or {}).get("results") if isinstance(body, dict) else None
        if not isinstance(results, list):
            raise OpenAlexResponseUnavailable("OpenAlex topic-search response was malformed")
        if not results:
            return None
        first = results[0]
        if not isinstance(first, dict):
            raise OpenAlexResponseUnavailable("OpenAlex topic-search result was malformed")
        tid = str(first.get("id") or "").rsplit("/", 1)[-1]
    ...
    def fetch_topic_works(self, conn: Connection, topic_id: str, *, cap: int = WORKS_SAMPLE) -> list[TopicWork]:
        ...
        try:
            return self.fetch_topic_works_strict(conn, topic_id, cap=cap)
        except OpenAlexResponseUnavailable:
            return []

    def fetch_topic_works_strict(self, conn: Connection, topic_id: str, *, cap: int = WORKS_SAMPLE) -> list[TopicWork]:
        """Return a complete bounded topic sample, raising when it cannot be established."""
        if not _TOPIC_RE.fullmatch(topic_id or ""):
            return []
        ...
        body = self._get(
            ...
            f"topicworks:{topic_id}:cap:{cap}",
            {"topic_id": topic_id, "cap": cap},
            strict=True,
        )
        results = (body or {}).get("results") if isinstance(body, dict) else None
        if not isinstance(results, list):
            raise OpenAlexResponseUnavailable("OpenAlex topic-work response was malformed")
        out: list[TopicWork] = []
        for work in results[:cap]:
    ...
    def _get(
        ...
        *,
        ...
        cached = get_cached(conn, OPENALEX_SOURCES_PROVIDER, cache_key, max_age_seconds=OPENALEX_CACHE_TTL_SECONDS)
        ...
        except Exception as exc:  # fail closed for compatibility; snapshot callers use strict=True
            if strict:
                raise OpenAlexResponseUnavailable(f"OpenAlex request {path} was unavailable") from exc
            return None
        if status == 200 and isinstance(body, dict):
            self._store(conn, cache_key, request_json=request_json, response_json=body, status_code=status)
            return body
        if strict:
            raise OpenAlexResponseUnavailable(f"OpenAlex request {path} returned HTTP {status}")
        return None
...
def _abstract_from_inverted_index(index: Any) -> str | None:
    ...
    for word, where in index.items():
        if isinstance(where, list):
            for position in where[:MAX_ABSTRACT_WORDS]:
...
def _source_meta(src: Any) -> SourceMeta | None:
    ...
    raw_issns = src.get("issn")
    issns = [str(x) for x in raw_issns if isinstance(x, str)] if isinstance(raw_issns, list) else []
    raw_concepts = src.get("x_concepts")
    concepts = [
        str(c.get("display_name"))
        for c in (raw_concepts[:8] if isinstance(raw_concepts, list) else [])
...
def _httpx_fetcher(
    ...
    response = bounded_openalex_get(OPENALEX_API_BASE + path, params=params, headers=headers, timeout=timeout)
    try:
        body = response.json()
    except ValueError:
        body = None
