...
VERDICTS: dict[str, tuple[str, bool, str, str]] = {
    ...
    "F26458": (
        ...
        (
            ...
            "Excluding it would remove legitimate scientific evidence."
        ),
        "adjudicated",
    ),
    # ---- math_or_symbol ----
    ...
    "F41636": (
        ...
        (
            ...
            "non-evidential, so the eligibility outcome is unaffected."
        ),
        "maintainer_reviewed",
    ),
    ...
    "F44485": (
        ...
        (
            ...
            "captured support for neuroessentialist/optimistic attitudes'. Evidential."
        ),
        "maintainer_reviewed",
    ),
    "F39057": (
    ...
    "F45476": (
        ...
        (
            ...
            "sentence, not a caption. Excluding it would remove real prose."
        ),
        "adjudicated",
    ),
    # ---- table_cell_debris ----
    "F29836": (
        C.TABLE_CELL_DEBRIS,
        False,
        ("'p = 0.146' -- a bare statistic with no referent. Real but non-propositional alone."),
        "maintainer_reviewed",
    ),
    ...
    "F34553": (
        ...
        (
            ...
            "heading. Misclassified, but non-evidential either way, so eligibility is unaffected."
        ),
        "adjudicated",
    ),
    ...
    "F41765": (
        C.ABSTRACT_PROSE,
        True,
        "Substantive prose about artistic production and neural pathways.",
        "adjudicated",
    ),
    # ---- body_prose ----
    ...
    "F27208": (
        ...
        (
            ...
            "non-evidential for a scientific claim either way."
        ),
        "adjudicated",
    ),
    ...
    "F35202": (
        C.BODY_PROSE,
        True,
        "Methods prose about participant compensation, mislabeled 'references'.",
        "adjudicated",
    ),
    ...
    "F36125": (
        ...
        (
            ...
            "region. Excluding it would remove legitimate scientific evidence."
        ),
        "adjudicated",
    ),
    ...
    "F25828": (
        C.BODY_PROSE,
        True,
        "Stimulus vignette text; substantive study material, not a reference.",
        "contestable",
    ),
    "F25864": (
        C.BODY_PROSE,
...
def main() -> None:
    ...
    print(
        f"frozen {len(frozen)} fixtures; {len(scored)} adjudicated/contestable, "
    ...
    for code in sorted(EXCLUDABLE):
        ...
        print(f"{code:<22}{len(tp):>4}{len(fp):>4}{n:>4}{len(tp) / n:>10.0%}   [{lo:.2f}, {hi:.2f}]   {len(harmful)}")
    ...
    fn = [f for f in scored if f["expected_type"] in EXCLUDABLE and f["predicted_type"] != f["expected_type"]]
    for f in fn:
    ...
    for f in scored:
        if f["adjudication"] == "contestable":
            print(
                f"  {f['fixture_id']} p{f['paper_id']} predicted={f['predicted_type']} "
