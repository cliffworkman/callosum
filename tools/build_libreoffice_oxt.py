...
`composer.py`, `citations_panel.py`, `evidence_insert.py`) that also serve as the by-hand macro install — so
there's a single source for both install routes. Pure stdlib (`zipfile`), so the backend can build it on demand
to serve / install (no Node, no shell). Every sibling module `callosum_cite.py` imports at runtime (currently
`composer.py`, `citations_panel.py`, and `evidence_insert.py`) must be listed in ENTRIES below, or a packaged
install (unlike the by-hand macro, which shares the same folder) 404s with "No module named '<name>'" the first
...
ENTRIES: list[tuple[str, Path]] = [
    ...
    ("evidence_insert.py", ADAPTER_DIR / "evidence_insert.py"),
]
