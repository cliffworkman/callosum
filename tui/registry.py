...
ACTIONS: tuple[Action, ...] = (
    ...
    Action(
        ...
        "/papers",
        params=(
            _q("q", help="search text"),
            _q("limit", "int"),
            _q("offset", "int"),
            _q("tag", help="filter by tag name"),
            _q("status"),
            _q("item_type"),
            _q("sort"),
            _q("order"),
    ...
    Action(
        "papers",
        "annotate",
        "Add annotation (agent: audited note)",
        "POST",
    ...
    Action(
        "papers",
        "edit",
        "Edit metadata (title/authors/year/…)",
        "PATCH",
        "/papers/{paper_id}",
        tier=WRITE,
        help="human-only; pass fields via --body JSON",
    ),
    Action(
        "papers",
        "read",
    ...
    Action(
        "papers",
        "export",
        "Export citations (bibtex/ris/csl-json)",
        "POST",
        "/papers/export",
        params=(
            _b("paper_ids", "json", required=True, help="e.g. [1,2,3]"),
            _b("format", required=True, help="bibtex|ris|csl-json"),
        ),
    ),
    Action("papers", "pdf", "Download PDF", "GET", "/papers/{paper_id}/pdf", help="binary; use --out FILE"),
    Action(
        "papers", "re-resolve", "Re-resolve metadata (Crossref)", "POST", "/papers/{paper_id}/re-resolve", tier=WRITE
    ),
    Action("papers", "fill-metadata", "Fill missing metadata", "POST", "/papers/{paper_id}/fill-metadata", tier=WRITE),
    ...
    Action(
        "papers", "delete-permanent", "Delete permanently", "DELETE", "/papers/{paper_id}/permanent", tier=DESTRUCTIVE
    ),
    Action("papers", "empty-trash", "Empty trash", "POST", "/papers/trash/empty", tier=DESTRUCTIVE),
    Action(
        "papers",
        "acquire-oa",
        "Acquire open-access PDF",
        "POST",
        "/papers/{paper_id}/acquire-oa",
        tier=WRITE,
        job="/papers/acquire-oa/{job_id}",
    ),
    Action(
        "papers",
        "citation-context",
        "Citation-context run",
        "POST",
    ...
    Action(
        ...
        job="/papers/citation-counts/refresh/{job_id}",
    ),
    Action(
        "papers",
        "save-reference",
        "Save a reference by DOI (audited)",
        "POST",
        "/agent/references",
        tier=WRITE,
    ...
    Action("discovery", "feed", "Show feed", "GET", "/feed", params=(_q("state"), _q("limit", "int"))),
    ...
    Action(
        ...
        "/feed/subscriptions",
        tier=WRITE,
        help="--body per feed kind (journal/author/query)",
    ),
    Action("discovery", "unsubscribe", "Remove subscription", "DELETE", "/feed/subscriptions/{sub_id}", tier=WRITE),
    Action("discovery", "feed-refresh", "Refresh feed now", "POST", "/feed/refresh", job="/feed/refresh/{job_id}"),
    Action(
        "discovery",
        "feed-state",
        "Set feed item state",
        "POST",
        "/feed/items/{item_id}/state",
        tier=WRITE,
        params=(_b("state", required=True, help="e.g. read|saved|dismissed"),),
    ),
    ...
    Action("wanted", "recheck", "Recheck OA availability", "POST", "/wanted/recheck", job="/wanted/recheck/{job_id}"),
    # ---- reading queue ----------------------------------------------------------
    ...
    Action(
        "tags",
        "add",
        "Tag a paper (agent: audited)",
        "POST",
        "/papers/{paper_id}/tags",
        tier=WRITE,
        agent_path="/agent/papers/{paper_id}/tags",
    ...
    Action(
        "tags",
        "axis-add-paper",
        "Add paper to axis (agent: audited)",
        "POST",
        "/axes/{axis_id}/papers",
        tier=WRITE,
        agent_path="/agent/axes/{axis_id}/papers",
        params=(_b("paper_id", "int", required=True),),
    ),
    Action(
        "tags", "axis-remove-paper", "Remove paper from axis", "DELETE", "/axes/{axis_id}/papers/{paper_id}", tier=WRITE
    ),
    Action("tags", "axis-order", "Reorder axes", "PUT", "/axes/{axis_id}/order", tier=WRITE),
    Action(
        "tags",
        "axis-score",
        "Score papers along axis",
        "POST",
        "/axes/{axis_id}/score",
        tier=WRITE,
        job="/axes/score/{job_id}",
    ),
    Action("tags", "axis-suggest", "Suggest new axes", "POST", "/axes/suggest", job="/axes/suggest/{job_id}"),
    ...
    Action("tags", "saved-search-remove", "Delete saved search", "DELETE", "/saved-searches/{search_id}", tier=WRITE),
    # ---- gaps & findings --------------------------------------------------------
    Action(
        "gaps",
        "list",
    ...
    Action(
        "gaps",
        "add",
        "Import a gap into library",
        "POST",
        "/gaps/add",
        tier=WRITE,
        agent_path="/agent/references",
        params=(
            _b("doi", required=True),
            _b("openalex_work_id"),
            _b("title"),
            _b("identifier", help="agent mode: DOI (only field sent)"),
        ),
    ...
    Action(
        "gaps",
        "finding-review",
        "Review a finding (FACT/CANDIDATE)",
        "POST",
    ...
    Action(
        "methods",
        "statcheck-run",
        "Statcheck the whole library",
        "POST",
        "/methods/statcheck/run",
        job="/methods/statcheck/run/{job_id}",
    ),
    Action("methods", "statcheck-summary", "Statcheck library summary", "GET", "/methods/statcheck/summary"),
    Action("methods", "bayes", "Bayes-factor re-analysis", "GET", "/papers/{paper_id}/bayes"),
    Action("methods", "grim", "GRIM test", "POST", "/methods/grim", help="--body {mean, n, decimals}"),
    Action(
        "methods",
        "effect-size",
    ...
    Action("methods", "pcurve", "P-curve analysis", "POST", "/methods/pcurve/run", job="/methods/pcurve/run/{job_id}"),
    Action("methods", "retraction", "Retraction status of a paper", "GET", "/papers/{paper_id}/retraction"),
    Action(
        "methods",
        "retraction-run",
        "Check library for retractions",
        "POST",
        "/methods/retraction/run",
        job="/methods/retraction/run/{job_id}",
    ),
    Action("methods", "retraction-summary", "Retraction library summary", "GET", "/methods/retraction/summary"),
    Action("methods", "retraction-db", "Retraction database status", "GET", "/methods/retraction/database"),
    Action(
        "methods",
        "retraction-db-refresh",
        "Refresh retraction database",
        "POST",
        "/methods/retraction/database/refresh",
        tier=WRITE,
        job="/methods/retraction/database/refresh/{job_id}",
    ),
    Action("methods", "transparency", "Transparency signals of a paper", "GET", "/papers/{paper_id}/transparency"),
    Action(
        "methods",
        "transparency-run",
        "Audit library transparency",
        "POST",
        "/methods/transparency/run",
        job="/methods/transparency/run/{job_id}",
    ),
    Action("methods", "transparency-summary", "Transparency library summary", "GET", "/methods/transparency/summary"),
    Action(
        "methods",
        "citation-equity",
        "Citation-equity audit",
        "POST",
        "/methods/citation-equity/run",
        job="/methods/citation-equity/run/{job_id}",
    ),
    Action(
        "methods",
        "citation-equity-overlooked",
        "Overlooked-work suggestions",
        "POST",
        "/methods/citation-equity/overlooked",
        job="/methods/citation-equity/overlooked/{job_id}",
    ),
    Action("methods", "lmm", "LMM-reporting audit", "GET", "/papers/{paper_id}/lmm"),
    Action("methods", "meta-analysis", "Meta-analysis extraction", "GET", "/papers/{paper_id}/meta-analysis"),
    Action(
        "methods",
        "publishers",
        "Where to submit (journal finder)",
        "POST",
        "/methods/publishers/run",
        job="/methods/publishers/run/{job_id}",
    ),
    # ---- citations ---------------------------------------------------------------
    ...
    Action(
        "citations",
        "render-document",
        "Render citations + bibliography",
        "POST",
        "/citations/render-document",
        help="--body {citations:[…], style, locale}",
    ),
    Action(
        "citations",
        "suggest",
    ...
    Action(
        ...
        "POST",
        "/summarize",
        job="/summarize/{job_id}",
        tier=WRITE,
        params=(_b("paper_id", "int", required=True),),
    ),
    Action("summaries", "reverify", "Re-verify a summary", "POST", "/summaries/{summary_id}/reverify", tier=WRITE),
    Action("summaries", "delete", "Delete a summary", "DELETE", "/summaries/{summary_id}", tier=DESTRUCTIVE),
    # ---- my publications ----------------------------------------------------------
    Action("mypubs", "profile", "Show profile", "GET", "/my-publications/profile"),
    Action("mypubs", "profile-set", "Set profile (ORCID…)", "PUT", "/my-publications/profile", tier=WRITE),
    Action("mypubs", "dashboard", "Publications dashboard", "GET", "/my-publications/dashboard"),
    Action(
        "mypubs",
        "refresh",
        "Refresh from ORCID/OpenAlex",
        "POST",
        "/my-publications/refresh",
        tier=WRITE,
        job="/my-publications/refresh/{job_id}",
    ),
    Action("mypubs", "decide", "Accept/reject a candidate work", "POST", "/my-publications/decide", tier=WRITE),
    Action(
        "mypubs",
        "domains",
        "Compute research domains",
        "POST",
        "/my-publications/domains",
        tier=WRITE,
        job="/my-publications/domains/{job_id}",
    ),
    Action("mypubs", "domain-rename", "Rename a domain", "POST", "/my-publications/domains/rename", tier=WRITE),
    Action("mypubs", "work-import", "Import a work", "POST", "/my-publications/works/import", tier=WRITE),
    Action("mypubs", "work-dismiss", "Dismiss a work", "POST", "/my-publications/works/dismiss", tier=WRITE),
    Action("mypubs", "work-undismiss", "Un-dismiss a work", "POST", "/my-publications/works/undismiss", tier=WRITE),
    Action(
        "mypubs",
        "summary-generate",
    ...
    Action("mypubs", "summary-set", "Edit research summary", "PUT", "/my-publications/summary", tier=WRITE),
    ...
    Action("mypubs", "citing-import", "Import a citing work", "POST", "/my-publications/citing/import", tier=WRITE),
    Action("mypubs", "reset", "Delete my-publications data", "DELETE", "/my-publications", tier=DESTRUCTIVE),
    # ---- library ops ---------------------------------------------------------------
    Action(
        "library",
        "scan",
        ...
        "/library/scan",
        tier=WRITE,
        job="/library/scan/{job_id}",
        params=(_b("path", required=True),),
    ),
    Action("library", "watched", "List watched folders", "GET", "/library/watched"),
    Action("library", "unwatch", "Remove watched folder", "DELETE", "/library/watched/{folder_id}", tier=WRITE),
    Action(
        "library",
        "rescan",
        "Rescan watched folders",
        "POST",
        "/library/watched/rescan",
        tier=WRITE,
        job="/library/watched/rescan/{job_id}",
    ),
    Action(
        "library",
        "enrich",
        "Refresh metadata enrichment",
        "POST",
        "/library/enrich/refresh",
        tier=WRITE,
        job="/library/enrich/refresh/{job_id}",
    ),
    Action(
        "library",
        "import",
    ...
    Action(
        ...
        tier=WRITE,
        job="/library/bundle/import/{job_id}",
    ),
    Action("library", "ocr", "OCR scanned PDFs", "POST", "/papers/ocr/run", tier=WRITE, job="/papers/ocr/run/{job_id}"),
    Action("library", "duplicates", "Find duplicates", "POST", "/papers/duplicates", job="/papers/duplicates/{job_id}"),
    Action("library", "duplicates-dismissed", "Dismissed duplicate pairs", "GET", "/papers/duplicates/dismissed"),
    Action(
        "library", "duplicates-dismiss", "Dismiss a duplicate pair", "POST", "/papers/duplicates/dismiss", tier=WRITE
    ),
    Action("library", "duplicates-undismiss", "Un-dismiss a pair", "POST", "/papers/duplicates/undismiss", tier=WRITE),
    Action(
        "library",
        "merge",
        "Merge two papers",
    ...
    Action("status", "access-token", "Mint remote-access token", "POST", "/settings/access-token", tier=WRITE),
    ...
    Action("status", "agent-revert", "Revert an agent write", "POST", "/agent/writes/{write_id}/revert", tier=WRITE),
    Action("status", "help-corpus", "Help corpus", "GET", "/help/corpus"),
    Action("status", "help-ask", "Ask the in-app help", "POST", "/help/ask", params=(_b("question", required=True),)),
)
