...
## 2026-07-29 — Increment 413: in-app feedback reporter (bug report / feature request)
<!-- HELP-DOCS-SYNCED 2026-07-29 inc 413 — help corpus gained "Reporting a bug or requesting a feature": the menu-bar Feedback reporter — the two kinds, the three ways to attach a screenshot (browser capture / paste / drag), the inspectable opt-out diagnostics ("what you see is exactly what is written"), and the honest send story (callosum writes the report to your machine and opens a draft; YOU press send), plus the blank-by-default destination in Settings → Feedback. This marker covers THIS increment's corpus change only — it does not certify the entries above the previous marker. -->
- **Files:** `app/backend/feedback/` (NEW — `bundle.py` 209, `diagnostics.py` 88, `destination.py` 50, `__init__.py`), `app/backend/api/routers/feedback.py` (NEW, 147), `app/backend/api/app.py` (+2, router mounted), `app/frontend/js/18b_feedback.jsx` (NEW, 345), `app/frontend/js/04b_workspaces.jsx` (+`FeedbackMenuItem` in both MenuBar branches), `app/frontend/js/35_settings.jsx` (+a "Feedback" SettingsCard), `app/frontend/styles.css` (+`.fb-*` + two mobile-menubar rules), `tests/test_feedback.py` (NEW, 18 tests), `.claude/qa-routes/route_76_feedback.md` (NEW), `.claude/security-audits/2026-07-29_feedback-reporter.md` (NEW, PASS), `app/backend/help/help_content.md` (+`reporting-a-bug`), `INCREMENT-413-NOTES.md` (NEW). `callosum-app.html` rebuilt. No DB change, no migration, no new dependency.
- **What:** a **Feedback** item beside Help/Settings/Status opens a reporter — bug vs feature, title/description/steps, an optional screenshot (browser `getDisplayMedia`, or paste/drag-drop), and an inspectable, opt-out diagnostics block. Submitting writes `~/.callosum/feedback/<stamp>_<kind>_<slug>/report.md` (+ the screenshot) and returns a **prefilled `mailto:` draft**; the destination address is **blank by default**, settable in the reporter or Settings → Feedback (env fallback `CALLOSUM_FEEDBACK_EMAIL`).
- **Why:** the app had no in-app way to tell its author something was broken — the thing a pre-1.0 tool with external adopters most needs. Built so it adds **no egress channel**: the server writes files and composes a URL, the user's own mail client does the sending, and the payload is readable before it moves (invariant #3 + inspectability). An unset address still saves the report rather than discarding it. Deliberately a **modal, not a utility workspace** — a bug report is about the screen you're looking at, and navigating away would discard the context (and the screenshot) being reported.
- **Verify:** `pytest tests/test_feedback.py` → 18 passed (incl. path-traversal, fake-PNG, oversized-image, table-injection, bounded-diagnostics); full suite **1689 passed, 1 skipped**; headless browser pass → 0 console/page errors, previewed diagnostics == written diagnostics, **zero** non-loopback requests from the feature, reachable + no overflow at 375px; line budget clean.
- **Open:** the Tauri shell has no `mailto:` handling (capabilities are `core:default`, no opener plugin) — "Open email draft" may be inert **inside the packaged desktop app**. Degrades honestly (the report is already on disk; Copy report / Copy folder path always work); flagged for the maintainer rather than assumed.
- **Revert:** `git revert <sha>` (additive: one new backend package + one router + one frontend chunk + small wirings; no migration). Bundles already written under `~/.callosum/feedback/` are user data and are left alone.
