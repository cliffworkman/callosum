...
<!-- HELP-DOCS-SYNCED 2026-07-04 inc 261 — help corpus gained "Building a CRediT contribution statement" (the CRediTer authoring aid: assign each author their NISO CRediT roles ± lead/equal/supporting → a by-author / by-role contributorship statement; callosum FORMATS what you assert and never infers/verifies who did what; Copy is universal, "Send to LibreOffice" stages it for the add-on's Insert-CRediT-statement, editing the grid clears the stage; credits tenzing + the CRediT/NISO taxonomy with a one-click library-add), and the "Where to submit" + "Review" sections were relabeled to live under the THEORY pane (moved from METHODS). Nothing above this line has an un-synced corpus change. -->
