...
- **Verify:** `pytest --ignore=tests/test_mcp_server.py` → **1031 passed, 1 skipped**; `ruff format`/`ruff check` clean; all touched `app/` files under the 600-line cap (`workbench.py` 374, `45_workbench.jsx` 347, `46_workbench_propose.jsx` 60); frontend built clean via esbuild; QA surface-map `check` clean (203/203 API + 965/965 FE, 0 uncovered). Manual script in `INCREMENT-259-NOTES.md` (port **8888**).
