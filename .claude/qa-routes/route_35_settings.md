...
8. **Unified provider roster (inc 149/150/256/547).** The AI-features section is ONE provider list (`GET /settings/providers`): **Gemini / OpenAI / Anthropic / Local AI / Local endpoint** plus user-added providers. A clean profile remains Gemini (Local AI must not silently become active). Cloud cards expose their masked key fields; Local endpoint exposes its loopback URL; managed Local AI exposes only **Set up Local AI**, on-device/privacy copy, evidence labels, status, and optional technical details. Click **Use** on OpenAI → it becomes Active and the model chooser appears; switching providers must reset the model override. Save a loopback URL on Local endpoint → accepted; non-loopback → **422**. With either local provider active and egress off, no cloud LLM host may be contacted.
   - **Managed Local AI happy path.** In packaged Windows Tauri with no cloud key, click **Set up Local AI**. Confirm the status progresses through download/verify/prepare to **Local AI: Ready**, then `GET /settings` reports `provider:"managed_local"`, `generation_provider_available:true`, Overview `evaluated`, and other generative capabilities `testing`. Generate an Overview, primary synthesis, and Help answer; all use the same managed loopback process and existing parsers/provenance. Restart the app and repeat a warm request without downloading again. Stop/exit leaves no llama-server process, token, or target descriptor; verified install files remain. Interrupt setup and retry; corrupt a same-size model copy in an isolated profile and retry; both must repair through a fresh verified partial/atomic promotion. During each failure/recovery capture traffic and prove zero Gemini/OpenAI/Anthropic calls.
