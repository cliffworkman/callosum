...
- **Managed Local AI Preview is a first-class, fail-closed provider (inc 547).** A clean profile still defaults to Gemini; managed Local AI is selected only after the user presses **Set up Local AI** and authenticated readiness succeeds. Setup must download the one pinned Windows runtime/model through allowlisted HTTPS, verify exact size/SHA-256/bundle identity, and atomically promote them. Once active, `GET /settings` reports `generation_provider_available:true` without a key or egress consent, and every provider-backed generation request resolves to the private Tauri descriptor. Missing/corrupt assets, runtime failure, timeout, or malformed output must never contact Gemini/OpenAI/Anthropic or show a cloud key as the required repair. A token/path in UI/API/logs, a non-127.0.0.1 bind, unauthenticated inference, partial file accepted as installed, cloud fallback, or orphan process is **Critical**.
...
- **Word add-in is local-first + bounded web relay (inc 164; category/batch/order/section/link surface incs 521-525; evidence inc 529).** The static `/integrations/word/*` routes serve FIXED bundled task-pane files + the manifest from `adapters/word/`; an undefined filename must be a plain 404 (no traversal). The one data route, `GET /integrations/word/evidence/{paper_id}`, is bearer-gated when Remote access is on, returns only four allowlisted saved-highlight fields, has no POST sibling, and exists so the method-blind tunnel never needs to expose the write-capable annotations path. The served `taskpane.html`/`taskpane.js` may reference Microsoft's `appsforoffice.microsoft.com` office.js (the required Office SDK) but **must not** reference any AI/library host (`generativelanguage`/`openai`/`anthropic`/`clffwrkmn.net`) — such a reference is **Critical**. The Word Node suite statically guards the in-pane controls; actual Office.js document behavior remains an explicit MANUAL Word check, not Playwright-drivable. `POST /integrations/word/install` opens a local folder and degrades to `{opened:false}`, never 500.
- **Packaged Word HTTPS is explicit, local-only, and removable (incs 532/534).** It defaults OFF. `POST /word-https/enable` and `/disable` require a literal-loopback request with no forwarding headers plus the Settings-only `X-Callosum-Local-Action` header; missing either must be **403**. Enable creates an end-entity certificate for only `localhost`/`127.0.0.1`, installs it only in the current user's trust store, and publishes enabled state only after trust verification. On Windows the protected Root store shows an ordinary certificate confirmation—not a UAC/elevation prompt—on both add and remove; declining/timing out must fail without false state. The Tauri-owned companion binds fixed `127.0.0.1:8443`, never `0.0.0.0`, and receives the Remote-access-off override only in that child. Disable removes trust before files/state. A private key/path in an API response, external request, enabled-by-default state, false cleanup success, or surviving companion after app exit is **Critical**. Browser/source mode must show the developer-certificate workflow honestly rather than claim Tauri supervision.
...
8. **Unified provider roster (inc 149/150/256/547).** The AI-features section is ONE provider list (`GET /settings/providers`): **Gemini / OpenAI / Anthropic / Local AI / Local endpoint** plus user-added providers. A clean profile remains Gemini (Local AI must not silently become active). Cloud cards expose their masked key fields; Local endpoint exposes its loopback URL; managed Local AI exposes only **Set up Local AI**, on-device/privacy copy, evidence labels, status, and optional technical details. Click **Use** on OpenAI → it becomes Active and the model chooser appears; switching providers must reset the model override. Save a loopback URL on Local endpoint → accepted; non-loopback → **422**. With either local provider active and egress off, no cloud LLM host may be contacted.
   - **Managed Local AI happy path.** In packaged Windows Tauri with no cloud key, click **Set up Local AI**. Confirm the one setup action visibly progresses through checking, runtime download/preparation, the main model download, verification, and authenticated startup to **Local AI: Ready**. Downloads show real MiB progress and an ETA only when measurable; verification/startup remain honestly indeterminate. The disabled action names the current phase rather than looking inert. Confirm `GET /settings` reports `provider:"managed_local"`, `generation_provider_available:true`, Overview `evaluated`, and other generative capabilities `testing`. Generate an Overview, primary synthesis, Help answer, Critical Read suggestion, and Funding Discovery triage; all use the same managed loopback process and existing parsers/provenance, including prompts larger than the old 4,096-token qualification context. Restart the app and repeat a warm request without downloading again. Stop/exit leaves no llama-server process, token, or target descriptor; verified install files remain. Interrupt setup and retry; corrupt a same-size model copy in an isolated profile and retry; both must repair through a fresh verified partial/atomic promotion. During each failure/recovery capture traffic and prove zero Gemini/OpenAI/Anthropic calls.
   - **An unavailable provider must say WHY (inc 568).** Whenever `GET /settings` reports `generation_provider_available:false`, it must also carry a `generation_provider_detail` sentence naming the cause, and that field must be `null` whenever generation IS available (so it can never read as a standing warning). Exercise all three classes: Local AI selected but the backend not started by the desktop app → the sentence names `app_data_missing` and points at the desktop app; Local AI selected but the model not running → `descriptor_unreadable`; a cloud provider with no key → the sentence names the missing API key, and with egress off → it names the missing consent. A bare `false` with no detail is the regression this exists to catch — five AI surfaces gate on that flag, and none of them could previously tell the user what to fix. The detail must never contain a key, token, or filesystem path.
...
13. **Microsoft Word add-in (incs 164/532/534).** In packaged Tauri, confirm **Microsoft Word add-in (desktop)** shows **Enable Word Support**, the local-only trust disclosure, **Download manifest**, and **Open Add-in Folder**. Before enable, `GET /word-https/status` reports `enabled:false`; click Enable, accept the ordinary Windows certificate **Security Warning** (no UAC elevation), and confirm the control becomes **Disable Word Support**, status reports ready/trusted, and `https://127.0.0.1:8443/health` is reachable with the exact generated certificate. Disable, accept the Root-store removal confirmation, and confirm trust/files are removed and the companion stops. Direct API negatives: a forwarded header or missing `X-Callosum-Local-Action: settings-ui-v1` on either mutation → **403**. In browser/source mode, confirm the developer `office-addin-dev-certs` + `tools/run_https.py` instructions appear instead of the packaged claim. The manifest remains non-empty with SourceLocation `https://localhost:8443/integrations/word/taskpane.html`; task pane external references remain limited to Microsoft office.js; Open Add-in Folder remains zero-egress and graceful; undefined files remain 404. *(Trust add/remove is live-verified; actual Office.js document behavior remains a consolidated MANUAL check.)*
