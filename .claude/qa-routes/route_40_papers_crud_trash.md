...
6. Soft-delete a disposable paper (`DELETE /papers/{paper_id}`). Confirm it leaves normal library view and appears in trash/deleted filter. **Discoverable per-card delete (backlog #57):** without checkbox-selecting anything, each library card shows an **always-visible** (not hover-only) `⋯` overflow trigger with an accessible label; it is keyboard-reachable, opens a menu with **Move to Trash**, closes on Escape/outside-click (returning focus to the trigger), and clicking the trigger or a menu item must **not** select or open the underlying card. "Move to Trash" goes through the existing confirm → soft-delete → Trash (recoverable) — a single accidental click never deletes. A hover-only trigger, a missing label, or a click that activates the card is a bug; batch delete must remain available and unchanged.
