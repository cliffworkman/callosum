...
21. **Inc 441: Checklists → Transparency wired to WIP manuscripts.** With a primary-file WIP open, go to Methods
    → Checklists → **Transparency signals**. Confirm the paper-wide/library controls are replaced by a manuscript-
    scoped **Check disclosures** action and an exact-checkpoint result. Run it against a draft containing a data
    disclosure plus at least one category the detector does not find. Confirm all seven statuses remain visible,
    the matched disclosure quote is labeled detected, the not-detected row plainly denies absence/score/judgment,
    and **Open source file** opens only the registered primary file. Confirm the same run appears in the manuscript's
    own Checks tab without reload; a detected fact has no review-disposition selector, while Statcheck candidates
    retain theirs. Change/rescan the primary file and confirm the prior run becomes potentially stale/stale. Repeat
    without a primary file and confirm the bounded 422 appears inline. Capture outbound requests with egress unset:
    the run must make no provider request. Switch back to a Library paper and confirm the original whole-library,
    per-paper, registration-reference, and Meta-Preregistration handoff remain unchanged.
22. **Inc 442: Checklists → Mixed-model reporting wired to WIP manuscripts.** With a primary-file WIP open, go to
    Methods → Checklists → **Mixed-model reporting**. Confirm the Library-wide controls are replaced by an explicit
    manuscript-scoped **Audit reporting** action. Run it against mixed-model text that reports some, but not all, of
    the seven items. Confirm the exact checkpoint/version/coverage and every status appear in both this tab and the
    manuscript's **Checks** tab; each **not found** row has one reviewable `info` candidate there, while present and
    n/a rows create no finding. Resolve all candidates and confirm the receipt remains inspectable and current without
    findings. Repeat with no mixed-model language and confirm a stored no-checklist-applied receipt with no findings
    and an explicit warning that the gate does not prove no mixed model. Change/rescan the primary file and confirm
    staleness; repeat without a primary file for the bounded 422. For a PDF, retain its real page/region evidence;
    for a non-PDF, do not show synthetic page precision. Capture outbound traffic with egress unset: no provider
    request may occur. Switch to a Library paper and confirm its batch/paper LMM workflow and lineage credit remain
    unchanged. Check both desktop and 375 px layouts and require zero console/page errors.
23. **Inc 443: Checklists → Bayesian statistics wired to WIP manuscripts.** With a primary-file WIP open, go to
    Methods → Checklists → **Bayesian statistics** and confirm Library-wide controls are replaced by an explicit
    manuscript-scoped audit. Use a draft containing one supported BF that reproduces, one gross default-prior
    mismatch, a prior statement, an MCMC convergence value crossing the displayed convention, a missing sensitivity
    statement, and one terminology advisory. Confirm the exact checkpoint/version/fixed assumptions/coverage plus
    every BF, checklist row, and advisory appear in both this tab and **Checks**. Confirm the mismatch, not-found/
    coherence rows, and advisory each create one separate reviewable `info` candidate; reproduced, present, and n/a
    rows create none. Resolve the candidates without deleting the receipt. Repeat with no Bayesian language and
    confirm a no-checklist/no-findings receipt that denies proving absence. Change/rescan for staleness; repeat with
    no primary file for 422. PDF evidence may retain real page/region; non-PDF evidence must clear synthetic pages.
    Capture outbound traffic: no provider request may occur. Switch to a Library paper and confirm its Bayesian batch,
    paper audit, and lineage credit remain unchanged. Require desktop/mobile layout and console/page-error assertions.
24. **Inc 444: Checklists → Meta-analysis reporting wired to WIP manuscripts.** With a primary-file WIP open, go to
    Methods → Checklists → **Meta-analysis reporting** and confirm the Library-wide controls are replaced by one
    manuscript-scoped audit over the registered primary file. Use a detectable meta-analysis that reports only the
    effect-size metric and model. Confirm both this tab and **Checks** retain the exact checkpoint/tool version/fixed
    coverage and all seven present/not-found/not-applicable rows. Confirm every `not-found` row creates exactly one
    separate reviewable `info` candidate; present and n/a rows create none. Resolve every candidate without deleting
    the receipt. Repeat with a within-study mini-meta-analysis for search-and-selection n/a, and with no meta-analysis
    language for a no-checklist/no-findings receipt that denies proving absence. Change and rescan for staleness;
    repeat without a primary file for 422. PDF evidence may retain real page/region while non-PDF evidence must clear
    synthetic pages. Capture outbound traffic: no provider request may occur. Switch to a Library paper and confirm
    its batch, paper audit, filter, findings, and lineage credit remain unchanged. Require desktop/mobile layout and
    zero console/page errors.
25. **Inc 445: Synthesize → Critique local grounded backbone for WIP.** With a primary-file WIP open, confirm
    Critique performs no automatic run and offers no provider candidate control. Start the local read explicitly.
    Confirm the receipt is exact-checkpoint-bound, uses only current-hash method receipts, lists at most 12 bounded
    draft claims, and records matching embedding/NLI/vector provenance. Seed a matching-model article-fulltext
    Library chunk and confirm a high-confidence contrast appears only as paired verbatim evidence with the other
    paper/attachment/page; its source jump opens that exact target and no defect finding is created. Supplements,
    registrations, deleted papers, and mismatched model versions must be ineligible. Confirm no-claim, empty-corpus,
    NLI/model-unavailable, and generic failure states are honest and retain the manuscript. Query embeddings must not
    be persisted. Change/rescan for staleness. Status must say Local AI, navigate by typed manuscript id, and contain
    no result text or path. Capture zero external traffic and verify desktop/375 px layouts without page errors.
