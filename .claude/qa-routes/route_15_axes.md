...
4. Run score (`POST /axes/{axis_id}/score`, `GET /axes/score/{job_id}`). Poll through pending/running/done. Navigate away during one run and return; no orphan spinner. Start several newly created axes scoring before the first finishes; every job completes without a duplicate-membership error, and each axis's cluster response contains each paper at most once even if concurrent first-time embedding work overlaps.
...
8. Open suggestion UI (`17_axes_suggest.jsx`). With egress unset, term suggestions and optimal-axis suggestions (`POST /axes/suggest-terms`, `POST /axes/suggest`, `GET /axes/suggest/{job_id}`) must fail closed with a clear egress-disabled message and no genai request.
   - **A11 (label provenance, inc 568):** a suggestion run that could NOT reach the configured provider must still complete (`status:"done"` with usable keyword labels) **and say so** — every `suggestions[].label_source` is `"keywords"` and `label_notice` names the cause (e.g. `app_data_missing` when Local AI is selected but the backend was not started by the desktop app). With a working provider the inverse must hold: `label_source:"ai"` and `label_notice:null`. A silent fallback — keyword labels with no notice — is the bug this assertion exists to catch; it previously read to the user as a model regression. The notice renders as `.axis-label-notice` (amber `--flag`, an unresolved *status*, never `--danger`) above the cards, which stay usable.
