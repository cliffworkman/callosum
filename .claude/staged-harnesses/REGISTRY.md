...
|---|---|---|---|
| [`pyright.md`](pyright.md) | Static type checking (strict, ratcheted) | A type-clean baseline exists across the core app, or before the first outside contributor's first typed module | drafted — **note:** its own trigger's second clause ("the first outside contributor's first typed module") has *also* fired by the same event that activated tach below (Jeffrey Vadala's fully-typed `tui/` PR #1, inc 472) — flagged for Cliff's own activation call, not activated unilaterally; a strict-pyright baseline pass is a separate, larger undertaking than tach's boundary rules |
| [`tach.md`](tach.md) | Module-boundary / import-layering rules | An outside contributor begins pushing code, or module count/coupling crosses a threshold that makes the file-containment rule (CLAUDE.md #1) hard to eyeball | **active** (inc 472) — `tach.toml`, `.pre-commit-config.yaml`, `.github/workflows/ci.yml`, `pyproject.toml`/`requirements-dev.txt`/`uv.lock`. Fenced: `app.backend.persistence` cannot import `app.backend.api`; `sync_server`/`mcp_server`/`tui` cannot import `app.backend` at all. Verified live with real, temporary negative-test violations (not just a clean pass) before shipping. |
