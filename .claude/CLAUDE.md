...
- **Cross-editor adapters — Microsoft Word and Google Docs (`adapters/word/`, `adapters/googledocs/`, incs
  ...
  **Word on the web**, riding the SAME cloudflared relay Google Docs already uses (`cloudflared-config.yml`
  ...
  `access_control.py`'s exemption-list fix). **Desktop Word live-verified 2026-08-28 (inc 508)** — the
  maintainer's first-ever real run: search-and-insert, Suggest-from-the-sentence, Refresh/renumber +
  bibliography, and Flatten all confirmed working. This surfaced and fixed two real, previously-undiscovered
  bugs: `tools/run_https.py` (invoked exactly as documented, `python tools/run_https.py`) raised
  `ModuleNotFoundError: No module named 'app'` — Python's script-mode `sys.path` doesn't include the project
  root, only the script's own directory, and this file never added it (every other `tools/` script needing a
  sibling `app` import already does); and `adapters/word/README.md`'s sideload instructions were wrong for
  ...
  verified-one-step-Undo/Redo transaction safety. **Inc 510** fixes a real bug live-testing inc 509 surfaced:
  ...
  *does* filter trash, confirmed by reading `paper_query_repo.py:399-409`), keying off response presence/count
  ...
  exists to support the latter) — disclosed, not silently dropped. See `INCREMENT-516-NOTES.md`. **Inc 517**
  (last self-built increment before handing the rest of the roadmap to Codex, per Cliff's own 48-hour usage
  window) does the accessibility pass: composer icon buttons (↑/↓/⋯/✕) gain `aria-label` alongside their
  ...
  note lifecycle remains explicitly **not yet live-verified**. See `INCREMENT-520-NOTES.md`. **Inc 521 starts
  bibliography item #11 with document-local categories:** the existing citations panel assigns one bounded label
  to a resolvable cited work, and Refresh groups the backend's already-rendered, identity-aligned bibliography
  entries alphabetically while preserving citeproc order inside each group and keeping unassigned/mixed entries
  under **Other references**. Missing alignment fails closed; a failed refresh restores the previous setting.
  No backend/rendering-semantic change. Pure category logic is Node-tested; Office.js settings/UI/layout remain
  explicitly **not yet live-verified**. See `INCREMENT-521-NOTES.md`. **Inc 522 adds bounded batch assignment:**
  ...
  **Inc 525 adds opt-in bibliography title/DOI links:** the existing backend's aligned, bounded HTTP(S) spans
  now flow through a document-local Word setting into both full and section bibliography blocks. The pure render
  plan preserves spans through category/order projection and converts Python Unicode-code-point offsets to
  JavaScript text safely. Word applies a link only after the generated entry paragraph and anchor each match
  exactly once; malformed, unsafe, shifted, or ambiguous metadata remains plain. Disable rebuilds only managed
  bibliography controls as plain text, so unrelated manuscript hyperlinks are untouched. No rendered text or
  ...
  **Inc 528 adds Word's bounded citation-coverage audit:** a read-only local scan maps inline and native-note
  citations to main-story paragraphs and reports only runs of 3+ substantive uncited paragraphs, excluding
  headings, tables, transitions, and managed bibliography blocks. It is a structural review prompt, not a support
  verdict or citation requirement, and duplicates no existing diagnostics/backend/model work. **Inc 529 adds
  author-controlled saved-evidence insertion:** search a paper's saved highlights, inspect exact quote/author note,
...
- **EndNote legacy-MyISAM reader and developer executor proven, production gated (backlog #57 Phase
  ...
  distribution. Inc 542 implements the approved developer-only executor outside production paths: it bounds and
  ...
  Inc 545 verifies official signed Linux binary/source provenance and a reproducible 31.8 MB stripped candidate,
  while establishing the non-negotiable license boundary: GPL-2.0-only MariaDB stays a separate optional process
...
| `.claude/docs/worktree-topology.md` | **Current-state (not historical) record of the checkout/worktree layout: which path is the canonical `main` checkout, where the frozen 0.6 baseline and the active Ask-CLI research branch live, and any other isolated in-progress worktree. Update in place when the topology changes; the *why* behind a change goes in `.claude/changes.md` instead.** |
