...
It is currently at **Increment 238** (see Increment workflow) with **884 pytest tests
...
│                                  threaded through the panels). The read-only *guarantee* = the CALLOSUM_READ_ONLY method gate.
...
[159] when the read-only additions took it over the 600 cap [HEAD was already 624]), `js/30_viewer.jsx` (~580), `repository.py` (**565**),
...
*Last updated: 2026-07-01 — increment 238 (B5 SP2 — **the read-only companion UI**; **B5 complete**; brainstormed →
