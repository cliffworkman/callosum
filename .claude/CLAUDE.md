...
It is currently at **Increment 371** (see Increment workflow) with **1555 pytest tests
...
- **Citations (formatted):** **citeproc-js** run as a Node sidecar (same subprocess pattern as esbuild) over
  ...
  **inc 366** validates and atomically installs local custom/dependent CSL styles outside the repository, making
  ...
  native Writer footnote numbering, including gaps from ordinary user-authored notes.
...
callosum is built in **numbered increments** (currently at 353). Each increment of real work
