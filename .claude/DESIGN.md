...
WIP checkpoint and tool-run status use the ordinary status vocabulary, not the WIP context color: `current` is
neutral because a matching hash is not verification; `current with findings`, `potentially stale`, and `stale` use
amber plus explicit text. Red remains reserved for destructive actions, and green remains reserved for
