...
- **#15 Sync — remaining threads.** [gated] Setup/enable/run UI, conflict review, and server hardening
  ...
  **Still open, not code:** the live deploy of `sync_server/` on Postgres + wiring the Authentik audience [your
  ...
  (share) shipped, inc 476**; still open: **SP4c** (receive — a "Shared with me" panel, decrypt-on-fetch,
  cross-user provenance, independent re-verification), **SP4d** (revoke/roles, with the limits of revocation
  disclosed honestly).
