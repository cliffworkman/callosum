...
- [x] **#63 Axis Suggest Status navigation** (inc 552) — the Status row now opens Theory → Axes and resumes its exact running or finished suggestion job instead of stopping at the pane or starting another run; see `increment-notes/INCREMENT-552-NOTES.md`.
- [x] **#66 Demo Discover-section staleness** (inc 555) — fixed `GET /feed/suggest-authors` having no demo route (Feed → Suggest → Author spun forever) and `missing_works`/`dismissed_works` being hardcoded empty instead of the real dashboard job's own values; see `increment-notes/INCREMENT-555-NOTES.md`.
- [x] **#67 Stale "three-paper" corpus-count copy sweep** (inc 555) — fixed ~15 sites across `demo/README.md`, `demo/coverage-v1.json`, several `app/frontend/js/*.jsx`, `demo/demo-runtime.js`, `www/*.html`, and `tools/demo/generate_demo_wip_state.py` after the corpus grew 3→5 papers; see `increment-notes/INCREMENT-555-NOTES.md`.
- [x] **#72 Local AI reachable from the dev browser build** (inc 569) — new developer-only `tools/run_local_ai.py` + `run_dev.py --local-ai` start the same llama-server from the already-installed, digest-reverified artifacts and publish a descriptor the *unmodified* `load_preview_target()` accepts, so Local-AI features are testable without an hours-long Tauri rebuild and with the desktop app closed; zero production code changed. Also fixed Settings reporting "Unsupported architecture" in any browser session. See `increment-notes/INCREMENT-569-NOTES.md` and `security-audits/2026-09-03_dev-local-ai-launcher.md`.
- [x] **#71 The Debian `.deb` installs but the app never opens** (inc 570) — the shell demanded `GLIBC_2.39` (two weak Rust-std symbols, `pidfd_spawnp`/`pidfd_getpid`) while Debian 12 ships 2.36, so the loader refused it outright; caused by `runs-on: ubuntu-latest` moving to Ubuntu 24.04. Diagnosed on a real Debian 12 box, which also disproved all three suspects the entry had listed. Shell now builds at the glibc floor (`ubuntu-22.04`) matching the runtime lane, with a blocking `objdump` guard; also fixed a second startup bug where a missing XDG Documents dir aborted `resolved_paths` before any spawn. See `increment-notes/INCREMENT-570-NOTES.md`.
- [x] **#74 The macOS verify step never reported the backend healthy** (inc 571) — cause was **macOS App Translocation**: the step quarantines the app to exercise Gatekeeper, and macOS runs a quarantined app from a randomised read-only `/private/var/folders/.../AppTranslocation/<uuid>/d/` copy; that instance stayed alive, so the later trusted `open` merely re-activated it and the health poll watched a process that was never the one under test (one PID persisted across probes 20 minutes apart). Stopping it before the trusted launch produced the first `healthy on port` macOS has ever printed — x64 45s, arm64 33s — and the gate is now blocking like Linux and Windows. Two hypotheses were disproven along the way (`ps` truncation; a hung `open`). Also closed the silences that hid it: `smoke_test` no longer discards the interpreter's output, `emit_status("failed")` now writes `startup-error.log`, and the startup task is supervised so a swallowed panic cannot pass as a slow launch. See `increment-notes/INCREMENT-571-NOTES.md`.
- [x] **#76 Local AI was Windows + macOS only; Debian/Ubuntu got "Unsupported architecture"** (inc 572) — Linux x86_64 now installs the same pinned `b10516` runtime, so a Linux user gets keyless Local AI like everyone else. Mostly a `#[cfg]` widening: `install_macos.rs` → `install_unix.rs` already generalised (per-platform allowlist, never one widened predicate), `process.rs`'s `#[cfg(unix)]` lifecycle inherited, `files.rs::is_runtime_library`'s dormant `.so` clause finally reached, and **no frontend change** — `preview.rs`'s `supported` predicate is the single switch. Two silent-breakage traps avoided: `ends_with(".so")` would have dropped all 29 versioned libraries and kept only their symlinks, and the bundle-manifest digest had to account for symlinks canonicalising to their targets — the algorithm was validated against the already-pinned *Windows* digest before being trusted for Linux. glibc checked first per #71 (runtime needs 2.34 vs our 2.35 floor) and pinned by a new blocking `objdump` guard that reads its pins out of `install.rs` rather than restating them. See `increment-notes/INCREMENT-572-NOTES.md` and `security-audits/2026-09-04_linux-local-ai.md`.
